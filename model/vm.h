	...
	*/
	...
	public:
		...
		*/
		...
		uint32_t m_vmid; // vm id
		uint32_t m_cpu; // cpu required for the vm
		uint32_t m_mem;  // memory required for the vm
		Ptr <Node> m_server; // host of the vm
		Ptr <VM> m_vmRemote; // communication between vmRemote and current vm
		...
		DataRate GetDataRate() {return m_cbrRate;};
		uint32_t CalculateMigCost();
	...
	private:
		...
		bool m_started; // check vm is started
